...
const ManualEntryModal = ({ isOpen, onClose, habits, onSave }: { isOpen: boolean, onClose: () => void, habits: any[], onSave: (entry: any) => void }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-6 animate-in fade-in duration-300">
      <div className="bg-white dark:bg-slate-900 p-10 rounded-[3rem] shadow-2xl w-full max-w-xl border border-slate-200 dark:border-slate-800">
        <div className="flex justify-between items-center mb-8">
          ...
          <button onClick={onClose} className="p-3 border border-slate-200 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-2xl text-slate-400 transition-all"><X /></button>
        </div>
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-4">
            <input 
              type="text" 
              className="w-full px-6 py-4 rounded-2xl bg-slate-50 dark:bg-slate-800 border-2 border-slate-200 dark:border-transparent focus:border-rose-500/50 outline-none text-slate-800 dark:text-white font-bold" 
              value={description}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <input type="date" className="w-full px-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 font-bold dark:text-white outline-none" value={date} onChange={e => setDate(e.target.value)} />
            </div>
            <div>
              <label className="block text-[10px] font-black text-slate-400 uppercase mb-2">From</label>
              <input type="time" className="w-full px-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 font-bold dark:text-white outline-none" value={startTime} onChange={e => setStartTime(e.target.value)} />
            </div>
            <div>
              <label className="block text-[10px] font-black text-slate-400 uppercase mb-2">Until</label>
              <input type="time" className="w-full px-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 font-bold dark:text-white outline-none" value={endTime} onChange={e => setEndTime(e.target.value)} />
            </div>
          </div>
          <button type="submit" className="w-full py-5 bg-rose-500 text-white rounded-[2rem] font-black uppercase tracking-widest text-xs mt-4 flex items-center justify-center gap-3 hover:bg-rose-600 shadow-xl shadow-rose-200 dark:shadow-none transition-all border border-rose-400 dark:border-transparent">
...
export const TimeTracker: React.FC = () => {
  ...
  return (
    <div className="space-y-10 max-w-6xl mx-auto animate-in fade-in duration-500">
      ...
      <div className="bg-white dark:bg-slate-900 p-6 rounded-[2.5rem] shadow-xl border border-slate-200 dark:border-slate-800 flex flex-col xl:flex-row items-center gap-6 sticky top-8 z-30 transition-all duration-500">
        ...
        <div className="flex flex-wrap items-center gap-6 w-full xl:w-auto justify-between xl:justify-end">
          <select className="bg-slate-50 dark:bg-slate-800 text-slate-700 dark:text-slate-200 px-6 py-3 rounded-2xl font-bold text-sm outline-none border border-slate-200 dark:border-slate-700 focus:border-blue-400" value={selectedHabitId} onChange={(e) => setSelectedHabitId(e.target.value)} disabled={!!data.activeTimer}><option value="">Choose Project</option>{allHabits.map(h => <option key={h.id} value={h.id}>{h.title}</option>)}</select>
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => setIsManualModalOpen(true)} className="p-4 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 rounded-2xl hover:bg-slate-200 transition-all border border-slate-200 dark:border-slate-700" title="Manual Entry"><PlusCircle size={24} /></button>
        ...
        {Object.entries(groupedEntries).map(([date, entries]) => (
          <div key={date}>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
              {entries.map((entry) => (
                <div key={entry.id} className="flex items-center justify-between p-6 border-b border-slate-200 dark:border-slate-800 last:border-b-0 hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-all gap-4">
