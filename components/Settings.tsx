    ...
    <div className="space-y-10 max-w-4xl mx-auto pb-12 animate-in fade-in duration-700">
      <div className="bg-white dark:bg-slate-900 rounded-[3rem] shadow-sm border border-slate-100 dark:border-slate-800 p-10">
        <div className="flex items-center gap-4 mb-8">
           <div className="p-4 bg-rose-50 dark:bg-rose-900/30 text-rose-500 rounded-2xl"><User size={24} /></div>
           <div>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4">
          <input type="text" value={tempName} onChange={(e) => setTempName(e.target.value)} className="w-full sm:flex-1 bg-slate-50 dark:bg-slate-800 border-2 border-transparent rounded-2xl px-6 py-4 text-slate-800 dark:text-slate-200 font-bold outline-none focus:border-rose-200 transition-all" />
          <button onClick={handleNameSave} className={`w-full sm:w-auto px-10 py-4 rounded-2xl font-black uppercase tracking-widest text-[10px] flex items-center justify-center gap-2 transition-all ${nameSaved ? 'bg-green-500 text-white' : 'bg-slate-800 dark:bg-rose-500 text-white hover:shadow-xl'}`}><Save size={16} /> {nameSaved ? 'Saved' : 'Update'}</button>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-[3rem] shadow-sm border border-slate-100 dark:border-slate-800 p-10">
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            <div className="p-4 bg-blue-50 dark:bg-blue-900/30 text-blue-500 rounded-2xl"><Bell size={24} /></div>
            <div>
          ...
          {permission !== 'granted' && <button onClick={requestPermission} className="px-6 py-3 bg-blue-500 text-white text-[10px] font-black uppercase tracking-widest rounded-2xl hover:bg-blue-600 transition-all">Enable</button>}
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {activeHabits.daily.length === 0 && <p className="text-slate-400 font-bold col-span-2 text-center py-6 border border-dashed border-slate-100 dark:border-slate-800 rounded-3xl">Create some habits to set reminders.</p>}
          {activeHabits.daily.map(habit => (
            <div key={habit.id} className="flex items-center justify-between p-6 bg-slate-50 dark:bg-slate-800/40 rounded-3xl group transition-all">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-[3rem] shadow-sm border border-slate-100 dark:border-slate-800 p-10 flex items-center justify-between">
        <div className="flex items-center gap-4">
           <div className="p-4 bg-purple-50 dark:bg-purple-900/30 text-purple-500 rounded-2xl">{theme === 'light' ? <Sun size={24} /> : <Moon size={24} />}</div>
